...
/*
 * Issues:
 * 1. ISR context crash.. suspecting that it happens when I don't protect something.. Happened on boot, before buttons, so guessing darkTimer
 * 2. 7 segement display flickers more as the brightness goes up. The panel draws more current than the regulator can provide.  Add a second regulator for the panel
 * 3. The display isn't working!!!  is it a bad pin?
 * /
...
time_t darkTime;
// int upButtonState; // these can be local
// int downButtonState;
...
void IRAM_ATTR onBluetoothTimer() {
  Serial.println("in the tiemr loop");
...

  {
    ...
    char str[20];
    memset(str, 0, 20);
    Serial.println("about to BT read");
